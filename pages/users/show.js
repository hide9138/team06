...
import styles from '../../styles/users/Show.module.css'
...
const contents = [BookShelf, Output, Output]
		...
		<div className={styles.main}>
			...
			<div className={styles.tabs}>
				{['本棚', 'アウトプット', 'いいね'].map((lable, i) => (
					<button key={i} onClick={() => setTabIndex(i)} className={`${styles.tab} ${tabIndex === i && styles.activeTab}`}>
						{lable}
					</button>
				))}
			</div>
			...
			{/* Tab contents */}
			<div className={styles.tabContentContainer}>{contents[tabIndex]()}</div>
		</div>
	)
}
