				...
				<a className={`${styles.nav__menu__link} ${href.includes(currentPath) && styles.nav__menu__link__active}`}>
					<div className={styles.nav__menu__image__container}>
						<Image
							...
							height="30"
							alt="icon"
							className={`${href.split('/')[1] === 'users' && styles.nav__menu__photo}`}
						/>
					</div>
...
const SideBar = () => {
	...
	const handleBookeDelete = () => {
		router.push('/books/delete')
	}
	...
	return (
		<div className={styles.sidebar}>
			...
			<div className={styles.btn__container}>
				<button className={styles.btn__output} onClick={handleBookeDelete}>
					本棚の整理
				</button>
			</div>

			<div className={styles.btn__container}>
				<button className={styles.btn__logout} onClick={handleLogoutButton}>
					ログアウト
				</button>
			</div>
		</div>
